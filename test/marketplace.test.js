  ...
  : describe('Marketplace unit tests', () => {
      ...
      describe('changing of contract params by owner', () => {
        it('reverts on execution by third person', async () => {
          ...
          await expect(marketplace.changeFeeWallet(attacker.address)).to.be
            .reverted;
          await expect(marketplace.changeBackendSigner(attacker.address)).to.be
            .reverted;
          await expect(marketplace.changeCurrencyAddress(attacker.address)).to
            .be.reverted;
          await expect(marketplace.changenftContract(attacker.address)).to.be
            .reverted;
          await expect(
            marketplace.changeCallTransactionAddresses([attacker.address]),
          ).to.be.reverted;
        });
